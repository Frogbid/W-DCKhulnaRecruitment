...
if (isset($_POST['login'])) {
   ...
   $query = "SELECT employee_id,password,roll_no,emp_img,id_card_img,certificate FROM employee_info_table where roll_no = '$roll_no' and password= '$password'";
   $data = mysqli_query($con, $query);
   $total = mysqli_num_rows($data);
   ...
   $sql = "SELECT employee_id,password,roll_no from employee_info_table where roll_no = '$roll_no' and password= '$password' ";
   ...
   } else if ($_POST['roll_no'] != $_POST['password']) {
   ...
   <div class="air__auth__authContainer">
      <div class="air__auth__containerInner">
         <div class="card air__auth__boxContainer">
            <div class="text-dark font-size-28 mb-3">লগ ইন করে প্রবেশ পত্র ডাউনলোড করুন</div>
            <div class="bg-danger text-white font-weight-bold mb-1 text-center">
               <?php echo $error_msg ?>
            </div>
            <form method="POST" class="mb-4">
               <div class="form-group mb-4">
                  <input type="number" name="roll_no" class="form-control" placeholder="Appliucant ID" required />
               </div>
               <div class="form-group mb-4">
                  <input id="passwordmatch" type="password" name="password" minlength="8" class="form-control" placeholder="enter password" required data-parsley-trigger="keyup" />
               </div>
